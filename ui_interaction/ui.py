"""
Módulo que maneja la interfaz de usuario en consola.
"""
...
def request_options() -> int | None:
    ...
    Returns:
        int | None: Opción seleccionada o None si inválida.
    """
    try:
        option = int(input())
        return option
...
def update_product(product_wait: Product) -> Optional[dict]:
    ...
    Args:
        product_wait (Product): Producto a actualizar.

    Returns:
        Optional[dict]: Cambios realizados.
    """
...
def logic_find_product(text: str):
    """
    Lógica para buscar, actualizar o eliminar un producto.

    Args:
        text (str): Acción ('buscar', 'actualizar', 'eliminar').
    """
    name: str = (
